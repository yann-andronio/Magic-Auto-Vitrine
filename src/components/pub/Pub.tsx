...
const Pub: React.FC = () => {
  ...
  return (
    <div
      className="w-full overflow-hidden shadow-xl py-4 z-50
      ...
      <Marquee gradient={false} speed={80} pauseOnHover={true}>
        {items.map((item, index) => (
          <a
            key={index}
            href={`#${item.label}`}
            className="flex items-center mx-6 whitespace-nowrap text-base font-semibold 
              text-gray-900 dark:text-white px-4 py-2 rounded-lg 
